  ...
  dbPromises.push(
    new Promise((resolve, reject) => {
      ...
      db.get(maintenanceSql, [tenantId], (err, row) => {
        if (err) return reject(err);
        dashboardData.maintenance = row || { pending_count: 0, in_progress_count: 0 };
        resolve();
      });
    })
  );
...
db.get(utilitiesSql, [tenantId], (err3, utilRow) => {
  ...
  const rateSql = `SELECT electricity_rate_per_unit FROM utility_rates ORDER BY effective_from DESC LIMIT 1`;
  db.get(rateSql, [], (errRate, rateRow) => {
    if (errRate) return res.status(500).json({ error: "Failed to fetch billing details" });
    const elecRate = (rateRow && typeof rateRow.electricity_rate_per_unit === 'number') ? rateRow.electricity_rate_per_unit : BILL_ELEC_RATE;
    const electricity_bill = elec_usage * elecRate;
...
// Convert a booking to a tenant (generate temp password, optional room assignment)



// Public: รับคำขอจากหน้าเว็บ
const bookingLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,  // 15 นาที
  max: 30,                   // ไม่เกิน 30 ครั้ง/ไอพี/ช่วงเวลา
  standardHeaders: true,
  legacyHeaders: false,
});

app.post("/api/public/bookings", bookingLimiter, (req, res) => {
  const b = req.body || {};
  const required = ["fullName", "phone", "email", "roomType", "moveIn"];
  ...
  const sql = `
    ...
    (full_name, phone, email, line_id, room_type, zone, room_no, move_in_date, duration_months,
     occupants, vehicle, contact_time, note, consent, status, source_ip, user_agent)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'new', ?, ?)`;
...
// === Public: room availability (by type) ===
...
const ROOM_TYPES = {
  ...
  air_built_in:{ key: "air_built_in",label: "ห้องแอร์บิวท์อิน", rent: 4500 },
};
...
// Create a deposit bill for a tenant (unpaid payment with today's due date)
app.post("/api/owner/tenants/:tenantId/deposit", requireOwner, (req, res) => {
  const tenantId = parseInt(req.params.tenantId, 10);
  const { amount, note } = req.body || {};
  if (!tenantId || amount == null) return res.status(400).json({ success:false, error:"missing_params" });
  const sql = `INSERT INTO payments (tenant_id, amount, status, due_date, note) VALUES (?, ?, 'unpaid', DATE('now'), json(?))`;
  const n = JSON.stringify({ deposit: true, note: note || null });
  db.run(sql, [tenantId, Number(amount) || 0, n], function (err) {
    if (err) return res.status(500).json({ success:false, error:"db_insert_failed" });
...
// ===== Bookings table (lead จากฟอร์มจอง) =====
function ensureBookingsTable(){
  const sql = `CREATE TABLE IF NOT EXISTS bookings (
    ...
    status           TEXT DEFAULT 'new',            -- new|contacted|scheduled|approved|rejected|converted
    scheduled_at     TEXT,
    staff_note       TEXT,
    source_ip        TEXT,
    user_agent       TEXT
...
ensureBookingsTable();
// Ensure extra columns for bookings
function ensureBookingsExtras(){
  db.all("PRAGMA table_info(bookings)", (err, rows) => {
    if (err || !Array.isArray(rows)) return;
    const cols = rows.map(r => r.name);
    const addCol = (name, ddl) => { if (!cols.includes(name)) db.run(`ALTER TABLE bookings ADD COLUMN ${ddl}`); };
    addCol('tenant_id', 'tenant_id INTEGER');
  });
}
ensureBookingsExtras();
...
app.post("/api/owner/bookings/:id/convert", requireOwner, (req, res) => {
  ...
  db.get("SELECT * FROM bookings WHERE id = ?", [id], (err, b) => {
    ...
    if (b.tenant_id) return res.json({ success:true, tenantId: b.tenant_id, alreadyConverted: true });

    // generate temp password
    const tempPass = ("bk" + String(id) + Math.random().toString(36).slice(2,8)).toLowerCase();

    const doCreate = (roomId) => {
      // check email duplicate
      db.get("SELECT id FROM users WHERE LOWER(email)=LOWER(?)", [b.email], (e1, ex) => {
        if (e1) return res.status(500).json({ success:false, error:"db_check_email" });
        if (ex)  return res.status(409).json({ success:false, error:"email_in_use" });

        const hashed = hashPassword(tempPass);
        db.serialize(() => {
          db.run("BEGIN");
          db.run("INSERT INTO users (name, email, phone, password, role) VALUES (?,?,?,?, 'tenant')",
            [b.full_name, b.email, b.phone, hashed],
            function (e2) {
              if (e2) { db.run("ROLLBACK"); return res.status(500).json({ success:false, error:"insert_user" }); }
              const newTenantId = this.lastID;
              const afterUser = () => {
                // update booking
                const note = (b.staff_note ? b.staff_note + "\n" : "") + `converted -> tenant_id=${newTenantId}`;
                db.run("UPDATE bookings SET status='converted', tenant_id=?, staff_note=?, updated_at=datetime('now') WHERE id=?",
                  [newTenantId, note, id], function (e4) {
                    if (e4) { db.run("ROLLBACK"); return res.status(500).json({ success:false, error:"update_booking" }); }
                    db.run("COMMIT");
                    res.json({ success:true, tenantId:newTenantId, tempPassword: tempPass, roomAssigned: !!roomId });
                  });
              };
              if (roomId) {
                // assign room to user and mark occupied
                db.run("UPDATE users SET room_id=? WHERE id=?", [roomId, newTenantId], function (e3) {
                  if (e3) { db.run("ROLLBACK"); return res.status(500).json({ success:false, error:"assign_room" }); }
                  db.run("UPDATE rooms SET status='occupied' WHERE id=?", [roomId], function (e3b) {
                    if (e3b) { db.run("ROLLBACK"); return res.status(500).json({ success:false, error:"occupy_room" }); }
                    afterUser();
                  });
                });
              } else {
    ...
    // resolve room id if room_no provided and available
    if (b.room_no) {
      db.get("SELECT id FROM rooms WHERE room_number = ? AND status = 'available' LIMIT 1", [b.room_no], (eR, room) => {
        if (eR) return res.status(500).json({ success:false, error:"db_room_lookup" });
        doCreate(room ? room.id : null);
      });
    } else {
      doCreate(null);
    }
...
app.get("/api/owner/bookings", requireOwner, (req, res) => {
  ...
  if (q) {
    where.push("(full_name LIKE ? OR phone LIKE ? OR email LIKE ? OR room_type LIKE ?)");
    params.push(`%${q}%`,`%${q}%`,`%${q}%`,`%${q}%`);
  }
  const sql = `
  ...
  db.all(sql, params, (err, rows) => {
    if (err) {
      ...
      return res.status(500).json({ success:false, error:"db_query_failed" });
    }
...
app.patch("/api/owner/bookings/:id", requireOwner, (req, res) => {
  const id = parseInt(req.params.id,10);
  const { status, staff_note, scheduled_at } = req.body || {};
  const fields = [];
  const params = [];
  if (status)       { fields.push("status = ?");       params.push(status); }
  if (staff_note!=null){ fields.push("staff_note = ?");   params.push(String(staff_note)); }
  if (scheduled_at) { fields.push("scheduled_at = ?"); params.push(String(scheduled_at)); }
  if (!fields.length) return res.status(400).json({ success:false, error:"nothing_to_update" });
...
// === Public: room availability (by type) ===
app.get('/api/public/room-availability', (req, res) => {
  const sql = `
    SELECT r.type,
           COUNT(*) AS total,
