...
// === Booking form (แทนที่ IIFE ของ contact form เดิม) ===
(function () {
  const bookingForm = document.querySelector('#contact form[data-role="booking"], #bookingForm');
  if (!bookingForm) return;

  const $f = (n) => bookingForm.querySelector(`[name="${n}"]`);
  ...
  bookingForm.addEventListener('submit', async (e) => {
    ...
    required.forEach((n) => {
      const el = $f(n);
      if (!el || !el.value || !String(el.value).trim()) errors.push(n);
    });

    // ตรวจสอบเบอร์โทร และอีเมล
    const phoneOk = /^0[0-9]{8,9}$/.test($f('phone')?.value || '');
    const emailOk = /^\S+@\S+\.\S+$/.test($f('email')?.value || '');
    if (!phoneOk) errors.push('phone');
    ...
    try {
      ...
      alert(`ส่งคำขอจองเรียบร้อย! หมายเลขอ้างอิง #${data.id}\nเราจะติดต่อกลับโดยเร็วที่สุด`);
      bookingForm.reset();
    } catch (err) {
  ...
  // เติมข้อมูลจากโมดัลรายละเอียดห้อง
  if (typeof mCTA !== 'undefined' && mCTA) {
    mCTA.addEventListener('click', () => {
      const roomId = (document.getElementById('m-title')?.textContent || '').match(/ห้อง (\w+)/)?.[1];
      const rec = ROOM_DB?.[roomId];

      if (roomId && bookingForm) {
        // โซนจากเลขห้อง
        const num = parseInt(roomId, 10);
        const zone = !isNaN(num) ? (num <= 7 ? 'old' : (num <= 14 ? 'mid' : 'new')) : '';


        if (rec && $f('roomType')) {
          $f('roomType').value = /บิ้วอิน/.test(rec.type) ? 'air+built' : 'air';
        }
...
// === Public: room availability counters ===
(async function () {
  try {
    const res = await fetch('/api/public/room-availability');
    if (!res.ok) throw new Error('fetch_failed');
    const data = await res.json();
    // Expect shape: { air: { total, vacant }, air_built_in: { total, vacant } }
    const map = {
      air: document.querySelector('[data-availability="air"]'),
      air_built_in: document.querySelector('[data-availability="air_built_in"]'),
    };
    Object.entries(map).forEach(([k, el]) => {
      if (!el) return;
      const rec = data[k] || { total: 0, vacant: 0 };
      if (rec.vacant <= 0) {
        el.textContent = 'ห้องเต็ม';
        el.classList.remove('bg-slate-100','text-slate-600');
        el.classList.add('bg-gray-200','text-gray-700');
      } else {
        el.textContent = `ว่าง ${rec.vacant} ห้อง`;
        el.classList.remove('bg-gray-200','text-gray-700');
        el.classList.add('bg-primary-100','text-primary-700');
      }
    });
  } catch (e) {
    const els = document.querySelectorAll('[data-availability]');
    els.forEach(el => el.textContent = 'อัปเดตไม่สำเร็จ');
    console.error('room-availability error', e);
  }
})();
